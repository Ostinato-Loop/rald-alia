// services/verification-service/src/index.ts
...
async function main(): Promise<void> {
  ...
  const server = app.listen(PORT, () => {
    logger.info({ port: PORT }, 'verification-service listening');
  });
...
main().catch((err) => {
  logger.error({ err }, 'verification-service fatal startup error');
  process.exit(1);
});
